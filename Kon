...
-- CFrame-step only within this distance. Beyond it, the caller should use
-- the game's island/portal teleport instead of crossing the whole map by CFrame.
local MAX_CFRAME_TARGET_DISTANCE = 300
...
local function getTargetDistance(target)
    ...
    if not hrp or not targetPart then return math.huge end
    return (targetPart.Position - hrp.Position).Magnitude
end
...
-- Returns true,"reached" when movement completed,
-- false,"too_far" when an island teleport should be used,
-- false,"timeout" when movement timed out.
local function moveToTarget(target, stopDistance, speed, teleportRadius, timeout)
    ...
    if getTargetDistance(target) > MAX_CFRAME_TARGET_DISTANCE then
    ...
    while target and target.Parent and os.clock() - started < timeout do
        ...
        if not hrp or not targetPart then
            ...
            if distance > MAX_CFRAME_TARGET_DISTANCE then
                ...
                mobKillThread = task.spawn(function()
                    ...
                    while mobKillActive do
                        ...
                        if currentTarget then
                            ...
                            local _, moveStatus = moveToTarget(currentTarget, 3, ENEMY_FLY_SPEED, ENEMY_TELEPORT_RADIUS, 0.2)
                            if moveStatus == "too_far" then
                                local portalId = GLOBAL_PORTAL_IDS[farPortalIndex]
                                pcall(function()
                                    RS.Remotes.TeleportToPortal:FireServer(portalId)
                                end)
                                TargetLabel:Set("Target: Too far, teleporting to " .. portalId .. "...")
                                farPortalIndex = farPortalIndex % #GLOBAL_PORTAL_IDS + 1
                                currentTarget = nil
                            else
                                fireM1()
                            end
        ...
        mobKillThread = task.spawn(function()
            ...
            while mobKillActive do
                ...
                if currentTarget then
                    ...
                    local _, moveStatus = moveToTarget(currentTarget, 3, ENEMY_FLY_SPEED, ENEMY_TELEPORT_RADIUS, 0.25)
                    if moveStatus == "too_far" then
                        local portalId = PORTAL_IDS[portalIndex]
                        pcall(function()
                            RS.Remotes.TeleportToPortal:FireServer(portalId)
                        end)
                        TargetLabel:Set("Current Target: Too far, teleporting to " .. portalId .. "...")
