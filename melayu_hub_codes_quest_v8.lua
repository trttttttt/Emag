...
-- Shared enemy/NPC movement: repeated CFrame-step teleport. There is no artificial
-- target-distance cap; if the target exists in workspace.Enemies it is chased directly.
local ENEMY_FLY_SPEED = math.clamp(tonumber(getSavedValue("EnemyFlySpeedSlider", 5)) or 5, 1, 200)
local ENEMY_TELEPORT_RADIUS = 50
-- No artificial CFrame distance cap. If a target exists in workspace.Enemies,
-- it is treated as being on the current loaded island and we CFrame-step directly
-- to it. Island/portal teleport is only used when the target is not present.
...
if game.PlaceId == ALLOWED_PLACE_ID then
        ...
        mobKillThread = task.spawn(function()
            ...
            while mobKillActive do
                ...
                if currentTarget then
                    ...
                    -- If the exact target is present in workspace.Enemies, it is on the
                    -- current loaded island. CFrame-step directly to it with no distance cap.
                    moveToTarget(currentTarget, 3, ENEMY_FLY_SPEED, ENEMY_TELEPORT_RADIUS, 12)
                    fireM1()
                else
    ...
    local function runAmbushLoop()
        ...
        while ffActive do
            ...
            if target then
                -- The target exists in workspace.Enemies on the current island.
                -- Distance is irrelevant: CFrame-step directly to it.
