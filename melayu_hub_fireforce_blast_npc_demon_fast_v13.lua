...
if game.PlaceId == ALLOWED_PLACE_ID then
    ...
    -- Dedicated Fire Force fly movement: uses velocity, not CFrame teleport.
    -- This has no distance cap; once the target is detected, the player flies
    -- toward it regardless of how far away it is.
    local function flyToTargetFF(mob)
        ...
        local savedCollision = setCharacterNoCollision(true)
        local hrp = getHRP()
        local humanoid = hrp and hrp.Parent and hrp.Parent:FindFirstChildOfClass("Humanoid")
        ...
        while ffActive and mob and mob.Parent and os.clock() - started < timeout do
            ...
            if not hrp or not targetPart then
                ...
                local direction = distance > 0 and offset.Unit or Vector3.zero
                -- Velocity-based flight. No CFrame assignment and no distance cap.
                hrp.AssemblyLinearVelocity = direction * math.max(speed * 8, 35)
                task.wait(0.03)
            end
        end
    ...
    local function runAmbushLoop()
        ...
        while ffActive do
            ...
            if target then
                -- Target found: fly directly to it. No CFrame cap and no
                -- CFrame teleport is used for Infernal Ambush.
                while ffActive and isFFTargetAlive(target) do
                    flyToAndKill(target)
                    task.wait(0.05)
                end
            else
                -- Nothing found: switch between the two allowed Fire Force
                -- islands and scan again.
