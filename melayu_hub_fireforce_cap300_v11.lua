...
local FIRE_FORCE_CFRAME_CAP = 300
-- Fire Force uses a hard CFrame movement cap; beyond this range its island
-- fallback should be used instead of crossing the map by CFrame. Island membership is checked separately so a
-- replicated enemy from another island never causes a cross-island CFrame chase.
...
if game.PlaceId == ALLOWED_PLACE_ID then
    ...
    local function runAmbushLoop()
        ...
        while ffActive do
            ...
            local hrp = getHRP()
            local targetPart = getTargetPart(target)
            local targetDistance = (hrp and targetPart) and (hrp.Position - targetPart.Position).Magnitude or math.huge

            if target and isTargetOnCurrentIsland(target) and targetDistance <= FIRE_FORCE_CFRAME_CAP then
                -- Target is on the current island and within the 300-stud CFrame cap.
                ...
                -- Missing target OR target is farther than 300 studs: do NOT
                -- CFrame across the map. Move to the other allowed Fire Force
                -- island and scan workspace.Enemies again there.
