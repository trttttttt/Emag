...
-- Shared enemy/NPC movement: repeated CFrame-step teleport. Fire Force applies
-- its own 300-stud pre-check before calling this function; other systems may use it
-- without that Fire Force-specific cap.
local ENEMY_FLY_SPEED = math.clamp(tonumber(getSavedValue("EnemyFlySpeedSlider", 5)) or 5, 1, 200)
local ENEMY_TELEPORT_RADIUS = 50
-- Fire Force Infernal Ambush uses flight movement to the target.
-- No Fire Force-specific CFrame distance cap is applied here.

local GLOBAL_PORTAL_IDS = {
...
local function getTargetPart(target)
    ...
    if not targetPart and target:IsA("Model") then targetPart = target.PrimaryPart end
    if not targetPart and target:IsA("BasePart") then targetPart = target end
    return targetPart
end

-- workspace.Enemies can contain replicated enemies from another island.
-- This is ONLY an island-membership guard; it is NOT a CFrame movement cap.
-- Once a target is classified as being on the current island, moveToTarget
-- has no distance limit and will CFrame-step to it regardless of range.
local CURRENT_ISLAND_SCAN_DISTANCE = 1500

local function isTargetOnCurrentIsland(target)
    local hrp = getHRP()
    local targetPart = getTargetPart(target)
    if not hrp or not targetPart then return false end
    return (hrp.Position - targetPart.Position).Magnitude <= CURRENT_ISLAND_SCAN_DISTANCE
end

-- Temporarily disable character collisions while CFrame-stepping so walls and
-- rocks do not push the character back between CFrame updates.
local function setCharacterNoCollision(enabled, savedCollision)
    local char = LP.Character
    if not char then return savedCollision end

    savedCollision = savedCollision or {}
    for _, obj in ipairs(char:GetDescendants()) do
        if obj:IsA("BasePart") then
            ...
            elseif savedCollision[obj] ~= nil then
                obj.CanCollide = savedCollision[obj]
            end
        end
    end
    return savedCollision
end

-- Returns true,"reached" when movement completed, or false,"timeout" if it times out.
-- The caller decides when an island/portal teleport is needed based on whether the
-- requested target exists in workspace.Enemies on the current island.
local function moveToTarget(target, stopDistance, speed, teleportRadius, timeout)
    if not target or not target.Parent then return false, "invalid" end
    ...
    local savedCollision = setCharacterNoCollision(true)
    ...
    while target and target.Parent and os.clock() - started < timeout do
        ...
        if not hrp or not targetPart then
            ...
            if distance <= teleportRadius then
                ...
                setCharacterNoCollision(false, savedCollision)
                return true, "reached"
            end
...
if game.PlaceId == ALLOWED_PLACE_ID then
                ...
                mobKillThread = task.spawn(function()
                    while mobKillActive do
                        ...
                        if currentTarget then
                            ...
                            -- A target returned by findNearestEnemy exists in workspace.Enemies,
                            -- so it is on the current loaded island. CFrame-step directly to it
                            -- regardless of distance; do not portal-hop just because it is far.
                            moveToTarget(currentTarget, 3, ENEMY_FLY_SPEED, ENEMY_TELEPORT_RADIUS, 12)
                            fireM1()
                        else
        ...
        for _, mob in ipairs(enemiesFolder:GetChildren()) do
            ...
            if hum and hum.Health > 0 and mobHRP then
                ...
                if actual == wanted then
                    local dist = (hrp.Position - mobHRP.Position).Magnitude
                    -- Ignore replicated enemies belonging to another island.
                    -- This does not limit CFrame movement after a valid target is found.
                    if dist <= CURRENT_ISLAND_SCAN_DISTANCE and dist < closestDist then
                        closest = mob
                        closestDist = dist
                    end
                end
    ...
    local function startQuestKillLoop()
        ...
        mobKillThread = task.spawn(function()
            ...
            while mobKillActive do
                ...
                if currentTarget and not isTargetOnCurrentIsland(currentTarget) then
                    -- The enemy exists in the replicated workspace.Enemies folder, but
                    -- it is not on our current island. Never CFrame across islands.
                    currentTarget = nil
                end
                ...
                if currentTarget then
                    ...
                    -- No CFrame distance cap here: a target classified as current-island
                    -- is approached by CFrame-step regardless of its distance.
                    moveToTarget(currentTarget, 3, ENEMY_FLY_SPEED, ENEMY_TELEPORT_RADIUS, 12)
                    fireM1()
                else
    ...
    local function findFFEnemy(targetName, maxDist)
        ...
        for _, mob in ipairs(enemiesFolder:GetChildren()) do
            ...
            if hum and hum.Health > 0 and mobHRP then
                ...
                if mobNameLower == nameLower then
                    local dist = (hrp.Position - mobHRP.Position).Magnitude
                    local islandMatch = dist <= CURRENT_ISLAND_SCAN_DISTANCE
                    if islandMatch and dist < closestDist and (not maxDist or dist <= maxDist) then
                        closest = mob
                        closestDist = dist
                    end
                end
    ...
    -- Fire Force target movement uses the same CFrame movement style as the
    -- main script: frequent CFrame updates with a snap once close enough.
    local function flyToTargetFF(mob)
        if not mob or not mob.Parent then return false, "invalid" end

        local started = os.clock()
        local timeout = 15
        local stopDistance = 4
        local speed = math.max(tonumber(ENEMY_FLY_SPEED) or 5, 1)
        local savedCollision = setCharacterNoCollision(true)

        while ffActive and mob and mob.Parent and os.clock() - started < timeout do
            local hrp = getHRP()
            ...
            if not hrp or not targetPart then
                ...
                local direction = distance > 0 and offset.Unit or Vector3.zero
                local travel = math.min(speed * 0.03, math.max(distance - stopDistance, 0))
                local nextPos = hrp.Position + direction * travel
                hrp.CFrame = CFrame.new(nextPos, targetPart.Position)
                task.wait(0.03)
            end
    ...
    -- ── Ambush: only 7th Company <-> Ruin City.  The detector follows the
    -- original script's logic: scan workspace.Enemies for a live matching
    -- target and pick the nearest one.  When it is not visible quickly,
    -- CFrame-teleport to the other island immediately instead of waiting
    -- several seconds trying to fly there.
    local function runAmbushLoop()
        local ambushIslands = { FF_ISLANDS.SEVENTH_COMPANY, FF_ISLANDS.RUIN_CITY }
        local islandCursor = 1
        local FAST_DETECT_WINDOW = 0.75
        local FAST_DETECT_INTERVAL = 0.05

        -- Put the player on the first allowed island immediately when the
        -- toggle starts, so the search begins from a known location.
        ...
        while ffActive do
            ...
            while ffActive and not target and os.clock() - searchStarted < FAST_DETECT_WINDOW do
                target = findFFEnemy("Infernal Ambusher")
                if not target then
                    task.wait(FAST_DETECT_INTERVAL)
                end
            end
            ...
            if target then
                ...
                -- No target on the current side: swap immediately and issue
                -- the Ambush request again on the new island.
                ...
                local swapStarted = os.clock()
                target = nil
                while ffActive and not target and os.clock() - swapStarted < FAST_DETECT_WINDOW do
                    target = findFFEnemy("Infernal Ambusher")
                    if not target then
                        task.wait(FAST_DETECT_INTERVAL)
                    end
                ...
                if target then
                    ...
                    -- Keep alternating without a long dead wait.
                    task.wait(0.05)
                end
            end
        end
    ...
    -- ── Boss: go to A-City, move to the White Whisperer NPC, then use the
    -- same SpawnBoss mechanic as the Boss Summon tab. Stopping the toggle
    -- cancels the loop cleanly; no summon loop is left running.
    local function teleportToWhiteWhisperer()
        local npcFolder = workspace:FindFirstChild("NPCs")
        local npc = npcFolder and npcFolder:FindFirstChild("White Whisperer")
    ...
    local function runBlastBossLoop()
        ...
        while ffActive do
            -- Re-position at the actual White Whisperer NPC before summoning.
            teleportToWhiteWhisperer()
            task.wait(0.15)

            -- Same server-side boss summon mechanic used by Boss Summon.
            pcall(function()
                RS.Remotes.Functions.Input:InvokeServer(
                    "SpawnBoss",
                    "White Whisperer",
            ...
            local searchTime = 0
            while ffActive and not target and searchTime < 4 do
            ...
            if target then
                ...
                -- Re-anchor at A-City/NPC on the next cycle if the boss did not
                -- appear in the current replicated enemy list.
                task.wait(0.15)
            end
        end
    end
    ...
    local function runDemonInfernalLoop()
        ...
        while ffActive do
            ...
            local searchTime = 0
            while ffActive and not target and searchTime < 2.5 do
                target = findFFEnemy("Demon Infernal")
                if not target then
                    task.wait(0.08)
                    searchTime = searchTime + 0.08
                end
            end
            ...
            if target then
                ...
                -- Faster island rotation when Demon Infernal is not found.
