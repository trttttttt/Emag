...
local function persistFlag(flagName, value)
    if ConfigReady and not ConfigHydrating then
...
-- Move toward an enemy/NPC like a low-speed flight. If the target is already
-- within 50 studs, snap beside it instead of slowly flying the last stretch.
local ENEMY_FLY_SPEED = math.clamp(tonumber(getSavedValue("EnemyFlySpeedSlider", 5)) or 5, 1, 100)
...
local function moveToTarget(target, stopDistance, speed, teleportRadius, timeout)
    ...
    speed = speed or ENEMY_FLY_SPEED
    ...
    while target and target.Parent and os.clock() - started < timeout do
        ...
        if not hrp or not targetPart then
            ...
            local travel = math.min(speed * 0.05, math.max(distance - stopDistance, 0))
...
if game.PlaceId == ALLOWED_PLACE_ID then
        ...
        while true do
            ...
            for flagName, element in pairs(Rayfield.Flags) do
                ...
                if ok and currentVal ~= nil then
                    ...
                    if encodedOk and lastSnapshot[flagName] ~= encoded then
    ...
    local function loadSavedFlagsInOrder(order)
        ...
        for _, flagName in ipairs(order) do
            if SavedConfig[flagName] ~= nil and Rayfield.Flags[flagName] then
        ...
        for flagName, _ in pairs(SavedConfig) do
            if not loaded[flagName] and Rayfield.Flags[flagName] then
    ...
    SettingsTab:CreateSlider({
        Name = "Enemy Fly Speed",
        Range = {1, 100},
        Increment = 1,
        CurrentValue = math.clamp(tonumber(getSavedValue("EnemyFlySpeedSlider", 5)) or 5, 1, 100),
        Flag = "EnemyFlySpeedSlider",
        Callback = function(value)
            ENEMY_FLY_SPEED = math.clamp(tonumber(value) or 5, 1, 100)
                        ...
                        if currentTarget then
                            ...
                            if notFoundTime >= 0.8 then
                                ...
                                if doubleCheck then
                                    ...
                                    notFoundTime = 0
                                    task.wait(0.6) -- give time to load into the island
                                end
                            end
                        end
    ...
    BossTab:CreateToggle({
        ...
        Callback = function(state)
            ...
            if state then
                ...
                Rayfield:Notify({ Title = "Boss Summon", Content = "Auto Summon + Fly Kill ON: " .. selectedBossLocation, Duration = 2 })
            else
                if autoSummonThread then
                    task.cancel(autoSummonThread)
                    autoSummonThread = nil
